...
- Started a third evidence-backed performance and discovery pass after a mobile Lighthouse baseline measured 5.3-second LCP and identified global animation-runtime and responsive-image waste.
- Replaced the globally loaded Framer Motion runtime with lightweight CSS transitions, added a 48 KB portrait mobile hero, an 8 KB header logo, and responsive 384/768-pixel pathway assets while preserving the existing visual design.
- Added an About, editorial and testing standards page; linked article authorship to it; added publication/modified dates, image dimensions, word counts, article sections, and consistent hydrated `WebApplication` schema for regional trainers.
- Added crawlable guide tables of contents and fragment-aware scrolling, marked ad/consent fallback copy `data-nosnippet`, aligned dedicated crawler exclusions for private routes, and added a verified IndexNow ownership/submission workflow for post-deployment discovery notifications.
- First compilation gate passes: application and Worker typechecks, production build, zero-vulnerability dependency audit, and IndexNow dry-run validation.

Follow-up prompt (2026-08-01): Adsterra ads register zero impressions. Auto-consent was requested so every visitor always sees ads with no cookie or ad consent prompt; find and fix the cause.

- Diagnosed the zero-impression cause on live production: `AdContext` started at `consent === null`, so `adsEnabled` was false for every first-time visitor, the "Your privacy choices" banner blocked the ad stack, and the page rendered house messages with no `/ad-frame` iframes, no native frame and no Social Bar script. Verified directly on `https://ninegatesmahjong.com` (`consent: null`, `0` ad iframes, `2` house slots, no social script).
- Confirmed the Adsterra inventory itself was healthy: `/ad-frame?placement=728x90` served a real paid creative and `/native-frame` rendered a 295-pixel native unit, so only the site's own consent gate suppressed revenue.
- Replaced consent gating with advertising enabled by default. `AdContext` now stores a preference rather than a consent choice, treats missing/unknown storage as enabled, tolerates blocked `localStorage`, and honors the legacy `declined` value as an opt-out.
- Deleted the interstitial `PrivacyChoices` banner and its dead stylesheet rules; the footer now carries a single low-key "Turn off ads on this device" / "Turn ads back on" control that keeps the privacy policy truthful without gating anyone by default.
- Fixed native-ad clipping: the native frame was constrained to a 100-pixel box while Adsterra rendered roughly 295 pixels. The Worker frame now reports its rendered height by same-origin `postMessage` and `NativeBanner` reserves 300 pixels and clamps growth to 800.
- Rewrote the advertising smoke test to assert the new contract: no consent prompt on first visit, Social Bar and both preconnect hints present by default, `/ad-frame` iframes rendered without interaction, opt-out clears all third-party advertising, and re-enabling clears stored opt-out.
- Local verification passes: application and Worker typechecks, production build, `test:features` (10 flows) and `test:site` (114 route/viewport checks, advertising frames, retired endpoints, security headers, zero failures). Browser check on the local Worker shows Social Bar loaded, three Adsterra preconnects, and 728x90, 320x50 and native frames all loading real ad markup with no prompt.
